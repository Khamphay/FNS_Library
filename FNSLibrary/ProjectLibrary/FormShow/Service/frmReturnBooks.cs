    ...
    public partial class frmReturnBooks : Form
    {
        ...
        private void ReturnBook()
        {
            try
            {
                ...
                MyMessageBox.ShowMesage("ບັນທືກສຳເລັດແລ້ວ", "Return", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Clear_Data();
        ...
        private void ShowBooks(string barcode)
        {
            try
            {
                table = new DataTable();
                table.Clear();
                da = new SqlDataAdapter("Select rentid, bid, barcode, bname, page, name, typename, qty, tbdid, datest, dateed, mbid, fname, lname, qty_total From vw_Return Where barcode='" + barcode+ "' And status=N'ກຳລັງຢືມ'", con);
                da.Fill(table);
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dgvReturnbooks.RowCount; i++)
                    {
                        if (dgvReturnbooks.Rows[i].Cells[0].Value.ToString() == table.Rows[i][2].ToString())
                        {
                            dgvReturnbooks.Rows.RemoveAt(i);
                            break;
                        }
                    }
                    ...
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                       // qty += int.Parse(table.Rows[0][7].ToString());
                        ...
                        if (DateTime.Now.CompareTo(DateTime.Parse(table.Rows[0][10].ToString())) > 0)
                        {
                            dgvReturnbooks.Rows[i].DefaultCellStyle.BackColor = Color.FromArgb(249, 207, 156);
                        }
                    }
                    ...
                    if (dateNow.CompareTo(dateExpire) == 1)
                    {
                        ...
                        txtbarcode.SelectNextControl(txtbarcode, false,true, true,true);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void txtbid_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            //txtbarcode.SelectNextControl((Control)sender, false, false, true, true);
        }
