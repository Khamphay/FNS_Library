...
namespace ProjectLibrary
{
    public partial class frmRentBook : Form
    {
        ...
        private void AddDataToDgv(string barcode)
        {
            try
            {
                cmd = new SqlCommand(@"Select tbBooks.barcode, tbBooks_Detail.bname, tbBooks_Detail.[page], tbCategory.[name], tbType.typename, tbBooks_Detail.tbdid, tbBooks.status  From tbBooks_Detail INNER JOIN tbBooks On tbBooks_Detail.bid = tbBooks.bid INNER JOIN tbCategory ON tbBooks_Detail.catgid = tbCategory.catgid INNER JOIN tbType ON tbBooks_Detail.typeid = tbType.typeid Where tbBooks.barcode='" + barcode+ "' And tbBooks.status=N'ຫວ່າງ'", con);
                ...
                if (dr.HasRows)
                {
                    ...
                    {
                        for(int i = 0; i < dgvRentbooks.RowCount; i++)
                        {
                            if(dgvRentbooks.Rows[i].Cells[0].Value.ToString()== dr["barcode"].ToString())
                            {
                                dgvRentbooks.Rows.RemoveAt(i);
                                break;
                            }
                        }
        ...
        private void FillData_ToLable()
        {
            try
            {
                cmd = new SqlCommand("Select fname, lname, tel, dateREd From tbMember Where mid='" + txtmemberid.Text + "' And dateREd>@dateEd And dateCardEnd>@date And status!=N'ບັດເສຍ'", con);
        ...
        private void LoadBookID()
        {
            try
            {
                auto = new AutoCompleteStringCollection();
                auto.Clear();
                cmd = new SqlCommand("Select barcode From tbBooks Where status=N'ຫວ່າງ'", con);
                dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    ...
                    for(int i = 0; i < dgvRentbooks.Rows.Count; i++)
                    {
                        ...
                        if (cmd.ExecuteNonQuery() == 1)
                        {
                            ...
                            if (cmd.ExecuteNonQuery() == 1)
                            {
                                ...
                                cmd = new SqlCommand("Update tbBooks_Detail Set rentQty+=1 Where bid='" + bid + "'", con);
                                cmd.ExecuteNonQuery();
                                ...
                                if (Reserve == true)
                                {
                                    ...
                                    if (cmd.ExecuteNonQuery() == 1)
                                    {
                                        ...
                                        if (cmd.ExecuteNonQuery() == 1)
                                        {
                                            cmd = new SqlCommand("if ('" + rsid + "') not in (Select tbReserve.rsid From tbReserve Where tbReserve.rsid = '" + rsid + "') Delete From tbReserve_Detail Where rsid = '" + rsid + "' else Update tbReserve_Detail Set qty -= 1 Where tbReserve_Detail.rsid = '" + rsid + "'", con);
                                            cmd.ExecuteNonQuery();
                                        }
                                    }
                                }
        ...
        private void btShowRent_Click(object sender, EventArgs e)
        {
            if (rentShow != null)
            {
                rentShow.Close();
            }
            rentShow = new frmShowRent();
            rentShow.Show();
        }
                ...
                if (indx > (-1))
                {
                    if (dgvRentbooks.Rows.Count > 0)
                    {
                        if (Reserve == true)
                        {
                            ...
                            if (cmd.ExecuteNonQuery() == 1)
                            {
                                cmd = new SqlCommand("Select tbBooks.bid From tbBooks inner join tbReserve On tbBooks.barcode=tbReserve.barcode Where tbReserve.barcode='" + dgvRentbooks.Rows[e.RowIndex].Cells[0].Value.ToString() + "'", con);
                                da = new SqlDataAdapter(cmd);
                                table = new DataTable();
                                table.Clear();
                                da.Fill(table);
