...
namespace ProjectLibrary
{
    public partial class frmAddNewBook : Form
    {
        ...
        private void MaxID(string bookid)
        {
            try
            {
                int index = 0;
                ...
                cmd = new SqlCommand("Select MAX(barcode) as maxid From tbBooks Where bid = N'" + bookid + "'", con);
                dr = cmd.ExecuteReader();
                dr.Read();
        ...
        private void Save()
        {
            if (txtid.Text != "" && txtid_Barcode.Text != "" && txtname.Text != "" && int.Parse(txtqty.Text) >= 1)
            {
                if (int.Parse(txtPage.Text) > 1)
                {
                    try
                    {
                        if (bid == "")
                ...
                {
                    MyMessageBox.ShowMesage("ກະລຸນາກວດສອບຈຳນວນໜ້າເຈ້ຍ ແລ້ວລອງໃໝ່ອີກຄັ້ງ!!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
        ...
        private void txtqty_TextChanged(object sender, EventArgs e)
        {
            if (txtqty.Enabled != false)
            {
                txtid_Barcode.Clear();
                int n = 0;
                if (txtqty.Text != "" && int.Parse(txtqty.Text)<=200)
                {
